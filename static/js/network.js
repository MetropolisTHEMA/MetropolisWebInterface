...
function drawLinkLegend(colorscale, min, max) {
  ...
  var bars = legendSvg.selectAll(".bars")
    ...
    .attr("y", function(d, i) {
      return ((intervals.length - 1 - i) * intervalHeight) + legendMargin;
    })
...
let subset_attributes = dataset.map(subset => [{
  lanes: subset.properties.lanes,
...
var speed_max = d3.max(dataset, function(d) {
  return d.properties.speed
});
var speed_min = d3.min(dataset, function(d) {
  return d.properties.speed
});
